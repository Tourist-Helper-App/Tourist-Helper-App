        ...
        if (process_payment($total, $_SESSION["phone_number"], $reference_id)) {
            ...
            try {
                ...
                foreach (array_map(null, $order['package_ids'], $order['package_counts'], $order['booking_dates']) as $package) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO booking (Order_ID, Package_ID, NumberOfTickets, Booking_Date) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$order_id, $package_id, $package_count, $booking_date]);
                }
                $pdo->commit();
